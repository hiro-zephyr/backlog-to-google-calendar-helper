...
async function ensureBacklogContentScript(tabId) {
  try {
    const response = await chrome.tabs.sendMessage(tabId, { type: 'PING_BACKLOG_GCAL' });
    if (response?.ok) return;
  } catch (error) {
    // content script が未注入の場合は明示注入する。
  }
...
async function prepareFromTab(tab, mode = 'calendar') {
  if (!tab?.id) return;
  ...
  try {
    await ensureBacklogContentScript(tab.id);
  } catch (error) {
    notifyTab(
      tab.id,
      `課題キー、URLのコピーに失敗しました。手動でコピーしてください。\n\n${error instanceof Error ? error.message : String(error)}`
    );
    return;
  }

  chrome.tabs.sendMessage(tab.id, { type: 'PREPARE_BACKLOG_FOR_GCAL', mode }, async (response) => {
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  if (command === 'open-backlog-to-gcal') {
    await prepareFromTab(tab, 'calendar');
    return;
  }

  if (command === 'open-backlog-to-gtask') {
    await prepareFromTab(tab, 'task');
    return;
  }
});
